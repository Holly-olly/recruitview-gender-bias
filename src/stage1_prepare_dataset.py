...
# Дедуп-флаг (2026-06-29): один и тот же кандидат (user_no) иногда переснимал ответ на тот же
# вопрос — 82 retake-строки в 75 группах (user_no × question_id). Чтобы избежать псевдо-
# репликации во within-question парном дизайне, оставляем ОДИН ответ на (user_no, question_id):
# с НАИБОЛЬШИМ id (даты съёмки нет — mp4 creation_time обнулён). dup_keep=False помечает retake.
# Все 2011 строк сохраняются (воспроизводимость Stage 1); Stage 2+ фильтрует df[df.dup_keep].
latest_id = df_clean.groupby(['user_no', 'question_id'])['id'].transform('max')
df_clean['dup_keep'] = df_clean['id'] == latest_id
n_drop = int((~df_clean['dup_keep']).sum())
print(f"\n2b. Дедуп: dup_keep=False у {n_drop} retake-дублей "
      f"(останется {len(df_clean) - n_drop} из {len(df_clean)})")
...
column_order = [
    ...
    'video_quality',
    'duration',
    'dup_keep',            # False = retake-дубль, исключать в Stage 2+ (см. CLAUDE.md)
    # Big Five
    'openness',
    'conscientiousness',
    'extraversion',
